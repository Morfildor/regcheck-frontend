...
function ResultsSidebarNav({ viewModel, children }) {
  ...
  // Not enough sections to warrant navigation — just render content
  ...
  return (
    <div className={layoutStyles.resultsWithSidebar}>
      {/* Left: sticky sidebar index (desktop only — hidden via CSS on mobile) */}
          ...
          <ol className={styles.sidebarNavList}>
            {flatSidebarItems.map((item) => {
              ...
              return (
                <li
                  key={item.id}
                  className={isSub ? styles.sidebarNavSubEntry : styles.sidebarNavEntry}
                >
                  <button
                    type="button"
                    className={cx(
      ...
      <div className={layoutStyles.resultsContentStack}>
        {/* Mobile chip row — hidden on desktop, shown on small screens via CSS */}
