...
const PRODUCT_TEMPLATES = [
  {
    label: "Air fryer",
    text: "Smart air fryer with Wi-Fi app control, mains powered, OTA updates, cloud recipe sync, and food-contact basket coating.",
  },
  {
    label: "Coffee machine",
    text: "Connected espresso machine with app control, mains powered, OTA updates, cloud brew profiles, water tank sensor, and food-contact brew path.",
  },
  {
    label: "Robot vacuum",
    text: "Robot vacuum cleaner with Wi-Fi and Bluetooth, LiDAR navigation, OTA firmware updates, cloud cleaning schedule, rechargeable lithium battery, and camera.",
  },
  {
    label: "Air purifier",
    text: "Smart air purifier with Wi-Fi control, PM2.5 sensor, OTA firmware updates, cloud air quality logging, and mains power.",
  },
];
...
function statusRank(s) {
  return { FAIL: 4, WARN: 3, PASS: 2, INFO: 1 }[s] || 1;
}
...
function prettyDirectiveName(key) {
...
function cleanVersionText(v) {
  return String(v || "").replace(/\s+/g, " ").trim();
}

function getStandardMeta(item) {
  return {
    harmonizedReference: cleanReference(
      item?.harmonized_reference || item?.harmonised_reference || ""
    ),
    harmonizedVersion: cleanVersionText(
      item?.dated_version ||
        item?.harmonized_version ||
        item?.harmonised_version ||
        ""
    ),
    stateOfTheArtVersion: cleanVersionText(
      item?.version || item?.state_of_the_art_version || ""
    ),
...
function inferStandardBadge(item) {
  ...
  const directive = String(
    item?.directive || item?.legislation_key || ""
  ).toUpperCase();
...
function buildStandardTags(item) {
  const directive = String(
    item?.directive || item?.legislation_key || "OTHER"
  ).toUpperCase();
...
function summarizeStandardCard(item, finding) {
  ...
  if (/EN\s*60335-1\b/.test(code))
    return "General household appliance electrical safety standard.";
  if (/EN\s*60335-2-\d+\b/.test(code))
    return "Product-specific Part 2 appliance safety standard.";
  if (/EN\s*55014-1\b/.test(code))
    return "EMC emissions requirements for household appliances and similar equipment.";
  if (/EN\s*55014-2\b/.test(code))
    return "EMC immunity requirements for household appliances and similar equipment.";
  if (/EN\s*61000-3-2\b/.test(code))
    return "Harmonic current emissions limits for mains-connected equipment.";
  if (/EN\s*61000-3-3\b/.test(code))
    return "Voltage fluctuation and flicker limits for mains-connected equipment.";
  if (/EN\s*61000-3-11\b/.test(code))
    return "Voltage changes, fluctuations and flicker limits for conditionally connected equipment.";
  if (/EN\s*61000-3-12\b/.test(code))
    return "Harmonic current limits for equipment with higher input current.";
  if (/EN\s*300 328\b/.test(code))
    return "Radio spectrum requirements for 2.4 GHz wideband transmission systems such as Wi-Fi and Bluetooth.";
  if (/EN\s*301 489-1\b/.test(code))
    return "Common EMC requirements for radio equipment.";
  if (/EN\s*301 489-17\b/.test(code))
    return "EMC requirements for 2.4 GHz and 5 GHz short-range radio technologies.";
  if (/EN\s*301 893\b/.test(code))
    return "Radio spectrum requirements for 5 GHz wireless access systems.";
  if (/EN\s*62311\b/.test(code))
    return "Assessment of RF exposure for electronic and radio equipment.";
  if (/EN\s*62479\b/.test(code))
    return "RF exposure assessment method for low-power electronic and radio equipment.";
  if (/EN\s*18031-1\b/.test(code))
    return "Cybersecurity requirements for protecting network functions of radio equipment.";
  if (/EN\s*18031-2\b/.test(code))
    return "Cybersecurity requirements for access control and protection against misuse.";
  if (/EN\s*18031-3\b/.test(code))
    return "Cybersecurity and personal data/privacy protection requirements.";
  if (/EN\s*63000\b/.test(code))
    return "Technical documentation standard for RoHS substance restriction compliance.";
  ...
  all.forEach((row) => {
    ...
    const itemPayload = {
      ...
      harmonized_reference:
        row.harmonized_reference || row.harmonised_reference || "",
      dated_version:
        row.dated_version ||
        row.harmonized_version ||
        row.harmonised_version ||
        "",
                ...
                <div style={{ display: "grid", gap: 7 }}>
                  {meta.harmonizedReference && (
                    <div style={{ fontSize: 13, color: "#6d675f", lineHeight: 1.55 }}>
                      <span style={{ fontWeight: 800, color: "#4c4741" }}>
  ...
  return (
    ...
    >
      ...
      <div style={{ minWidth: 0, flex: 1 }}>
        ...
        {finding.action && (
          <div style={{ marginTop: 4, fontSize: 13, color: "#7a746b", lineHeight: 1.55 }}>
            {finding.action}
          </div>
        )}
      </div>
...
function EmptyState({ title, subtitle }) {
  return (
    <div
      style={{
        borderRadius: 12,
        border: "1px dashed rgba(186,176,160,0.7)",
    ...
    >
      ...
      {subtitle && (
        <div style={{ marginTop: 6, fontSize: 14, lineHeight: 1.55 }}>{subtitle}</div>
      )}
    </div>
  );
}
      ...
      return fetch(API_URL, {
        ...
        .then((res) =>
          res.json().then((data) => {
            ...
            buildGroupsFromBackendItems(data.standards || [], data.review_items || []).forEach(
              (group) => {
  ...
  return (
    <div
      style={{
        minHeight: "100vh",
        background:
          "radial-gradient(ellipse at top left, rgba(133,176,190,0.25), transparent 30%), radial-gradient(ellipse at top right, rgba(204,189,148,0.2), transparent 28%), linear-gradient(180deg,#ddd7cf 0%,#d4cdc4 100%)",
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
              ...
              <button
                ...
                style={{
                  ...
                  boxShadow:
                    loading || !text.trim() ? "none" : "0 6px 20px rgba(86,122,130,0.25)",
                ...
                <div style={{ padding: "14px 14px", display: "grid", gap: 10 }}>
                  {!filteredStandardGroups.length ? (
                    <EmptyState
                      ...
                      subtitle="Run analysis with a more detailed product description."
                    />
                  ) : (
                    DIR_ORDER.filter((d) => standardsByDirective[d] && standardsByDirective[d].length).map(
                      (directive) => (
                        <DirectiveLane
                          key={directive}
                          dirKey={directive}
